...
{
public:
    PImpl(VideoEncoder *owenr, AVCodecID id, EncoderConfig cfg);
    ~PImpl() = default;
...
public:
    VideoEncoder       *owenr_ = nullptr;
    const AVCodec      *codec_ = NULL;   ///< 编码器
    CodecContextWrapper ctx_{ nullptr }; ///< 编码器上下文>
...
VideoEncoder::PImpl::PImpl(VideoEncoder *owenr, AVCodecID id, EncoderConfig cfg) :
    owenr_(owenr), codec_id_(id), config_(std::move(cfg))
{
}
