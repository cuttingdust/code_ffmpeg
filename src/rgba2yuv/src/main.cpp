...
#define YUV2_FILE ".\\assert\\out_400_300_25.yuv"
...
int main(int argc, char *argv[])
{
    ...
    /// 一定要以二进制打开
    ifs.open(RGBA_FILE, std::ios::binary);
    ofs.open(YUV2_FILE, std::ios::binary | std::ios::app);
    ...
    for (;;)
    {
        ...
        /// YUV转RGBA
        /// 上下文件创建和获取
        yuv2rgb = sws_getCachedContext(rgb2yuv,               /// 转换上下文，NULL新创建，非NULL判断与现有参数是否一致，
                                                              /// 一致直接返回，不一致先清理当前然后再创建
